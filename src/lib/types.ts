...
import { lowerFirstChar, sentenceJoin } from './utils';
...
export function appartmentShortDescription(appartment: Appartment): string {
	let descriptionPlainText = html2text(appartment.description);
	return (
		DISPLAY_APPARTMENT_KIND[appartment.kind] +
		' avec ' +
		sentenceJoin(
			(
				Object.entries(DISPLAY_ASPECT_FIELDS) as [
					keyof typeof DISPLAY_ASPECT_FIELDS,
					string
				][]
			)
				.filter(([key, _]) => appartment[key])
				.map(([_, display]) => display)
		) +
		'. ' +
		descriptionPlainText.slice(0, 70) +
		(descriptionPlainText.length > 70 ? '…' : '')
	);
}
